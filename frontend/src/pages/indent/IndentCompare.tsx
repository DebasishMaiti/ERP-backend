...
import { Save, Send, RotateCcw, Wand2, ExternalLink, AlertTriangle, DollarSign, Eye, ChevronRight, ArrowLeft, Truck } from "lucide-react";
...
export default function BoQCompare() {
  ...
  // Filter BoQs that are ready for comparison (status "Compare")
  const comparePendingBoqs = mockData.boqs.filter(b => b.status === "Compare" || b.nextStep === "Compare & Select");
        ...
        <div className="container mx-auto px-4 py-6">
          <Card>
            <CardContent className="text-center py-8">
              <p className="text-muted-foreground">This page is only accessible to Purchasers, Admins, and Accountants.</p>
            </CardContent>
          </Card>
        </div>
      </div>;
  ...
  const validateSelections = () => {
    ...
    selections.forEach((item, index) => {
      if (!item.hasActiveVendors) return; // Skip items without vendors
      ...
      // Only need reason if vendor is NOT lowest (don't include below MOQ in reason requirement)
  ...
  const hasValidationErrors = useMemo(() => {
    return validateSelections().length > 0;
  }, [selections]);
  ...
  const proceedToApproval = useCallback(() => {
    ...
    // In real app, this would update BoQ status and navigate to approval
    toast({
      title: "Success",
  ...
  const openCompareDetail = useCallback((boqId: string) => {
    setSearchParams({
      boqId
    });
  ...
  // Calculate coverage for each BoQ
  const getBoqCoverage = useCallback((boq: any) => {
    const itemsWithVendors = boq.items.filter((boqItem: any) => {
      const itemMaster = items.find(item => item.id === boqItem.itemId || item.name === boqItem.name);
      return itemMaster && itemMaster.vendors.some((v: any) => v.active);
    });
    return {
  ...
  const renderCompareQueue = () => <div>
      
      <div className="container mx-auto px-4 py-6">
        {comparePendingBoqs.length === 0 ? <Card>
            ...
            return <Card key={boq.id} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => openCompareDetail(boq.id)}>
                        <CardContent className="p-2">
                          <div className="flex justify-between items-start mb-1">
                            <div>
                              <h4 className="font-medium text-sm">{boq.number}</h4>
                           ...
                           <div className="space-y-1 text-xs">
                             <div className="flex justify-between">
                               ...
                               <span className="font-medium">{boq.project}</span>
                             </div>
                             <div className="flex justify-between">
                               <span className="text-muted-foreground">Items:</span>
                               <span>{boq.itemCount}</span>
                             </div>
                            <div className="flex justify-between">
                            ...
                            {missingPriceCount > 0 && <div className="flex items-center gap-1 text-amber-600">
                ...
                </div> : <Table className="rounded-md border bg-white">
                  <TableHeader>
                    <TableRow>
              ...
              const coverage = getBoqCoverage(boq);
              const missingPriceCount = coverage.total - coverage.covered;
              return <TableRow key={boq.id} className="cursor-pointer hover:bg-muted/50" onClick={() => openCompareDetail(boq.id)}>
                          ...
                          <TableCell>
                            <div>
                              ...
                              <div className="text-sm text-muted-foreground">{boq.createdOn}</div>
                            </div>
                          </TableCell>
                          ...
                          <TableCell>
                            {missingPriceCount > 0 && <div className="flex items-center gap-2 text-amber-600">
                                <AlertTriangle className="h-3 w-3" />
                                <span className="text-xs">Missing price for {missingPriceCount} items</span>
                              </div>}
                          </TableCell>
  ...
  if (!boq) {
    return <div>
        <div className="container mx-auto px-4 py-6">
          <Card>
            <CardContent className="text-center py-8">
              <p className="text-muted-foreground">Please check the BoQ ID and try again.</p>
      ...
      <div className="container mx-auto px-2 sm:px-4 py-4 sm:py-6">
        <div className={`${isMobile ? 'space-y-4' : 'grid grid-cols-3 gap-6'}`}>
          ...
          <div className={isMobile ? '' : 'col-span-2'}>
            {/* Action Buttons */}
            {!isReadOnly && <div className={`${isMobile ? 'grid grid-cols-2 gap-2 mb-4' : 'flex flex-wrap gap-2 mb-6'}`}>
                <Button variant="outline" onClick={autoSelectLowest} size={isMobile ? "sm" : "default"} className={isMobile ? "text-xs" : ""}>
                  <Wand2 className="h-4 w-4 mr-1" />
                  {isMobile ? "Auto-select" : "Auto-select Lowest (All)"}
                </Button>
                <Button variant="outline" onClick={clearAllSelections} size={isMobile ? "sm" : "default"} className={isMobile ? "text-xs" : ""}>
            ...
            <div className={`${isMobile ? 'space-y-3' : 'space-y-6'}`}>
              {selections.map((item, itemIndex) => <Card key={item.itemId} className="shadow-card overflow-hidden">
                  <CardHeader className={isMobile ? "pb-3" : ""}>
                    <div className="flex justify-between items-start">
                      <div className="flex-1 min-w-0">
                        <CardTitle className={`${isMobile ? 'text-base' : 'text-lg'} line-clamp-2`}>{item.itemName}</CardTitle>
                        <p className={`${isMobile ? 'text-xs' : 'text-sm'} text-muted-foreground`}>
                          {item.quantity} {item.unit}
                        </p>
                      </div>
                      {item.selectedVendor && <Badge variant="outline" className={`${isMobile ? 'text-xs ml-2 shrink-0' : ''}`}>
                  ...
                  <CardContent className={isMobile ? "pt-0" : ""}>
                    {!item.hasActiveVendors ? <div className={`text-center ${isMobile ? 'py-4' : 'py-6'}`}>
                        ...
                        <RadioGroup value={item.selectedVendor || ""} onValueChange={vendor => !isReadOnly && updateSelection(itemIndex, vendor)} disabled={isReadOnly}>
                      ...
                      return <div key={option.vendor} className="space-y-2">
                                    ...
                                    <div className={`${isMobile ? 'space-y-2' : 'flex justify-between items-center'}`}>
                                      <div>
                                        <div className="flex items-center gap-2 mb-1">
                                          ...
                                          {option.isLowest && <Badge variant="default" className="text-xs">
                                              Lowest
                                            </Badge>}
                                        </div>
                                      </div>
                                ...
                                {/* Override Reason */}
                                {isSelected && needsReason && !isReadOnly && <div className={`${isMobile ? 'ml-0 mt-2' : 'ml-6'} space-y-2`}>
                                    <Label className={`${isMobile ? 'text-xs' : 'text-sm'} text-muted-foreground font-medium`}>
                                      Reason for not selecting lowest price *
                                    </Label>
                                    <Textarea value={item.overrideReason} onChange={e => updateOverrideReason(itemIndex, e.target.value)} placeholder="Enter reason for override..." rows={2} className={`${isMobile ? 'text-sm min-h-[60px]' : 'text-sm'}`} />
                                  </div>}
                              </div>;
                    })}
                        </RadioGroup>
            ...
            {/* Mobile Actions */}
            {isMobile && !isReadOnly && <div className="sticky bottom-20 bg-background p-3 border rounded-lg shadow-lg space-y-2 mx-2">
            ...
            <Card className={`${isMobile ? '' : 'sticky top-6'}`}>
              ...
              <CardContent className={`${isMobile ? 'space-y-3' : 'space-y-4'}`}>
                 {poPreview.length === 0 ? <p className={`text-muted-foreground ${isMobile ? 'text-sm' : 'text-sm'}`}>
                    ...
                    {poPreview.map((vendor, index) => <div key={vendor.vendor} className={`${isMobile ? 'space-y-2' : 'space-y-3'}`}>
                        ...
                        <div className={`${isMobile ? 'space-y-2 p-2' : 'space-y-2 p-2'} bg-muted/30 rounded-md`}>
                          <div className="flex items-center justify-between">
                            ...
                            {!isReadOnly && <Button variant="ghost" size="sm" onClick={() => toggleFleetCostEditing(vendor.vendor)} className={`${isMobile ? 'h-6 px-2 text-xs' : 'h-6 px-2 text-xs'}`}>
                                {fleetCostEditing[vendor.vendor] ? 'Cancel' : vendor.fleetCost > 0 || vendor.fleetGst > 0 ? 'Edit' : '+ Add'}
                              </Button>}
                          </div>
                          
                          {!isReadOnly && fleetCostEditing[vendor.vendor] ? <div className="space-y-2">
                              <div className={`${isMobile ? 'grid grid-cols-2 gap-2' : 'flex gap-2'}`}>
                                <div className="flex-1">
                                  ...
                                  <Input id={`fleet-cost-${vendor.vendor}`} type="number" placeholder="0" value={fleetCosts[vendor.vendor]?.cost || ''} onChange={e => updateFleetCost(vendor.vendor, parseFloat(e.target.value) || 0, fleetCosts[vendor.vendor]?.gst || 0)} className={`${isMobile ? 'h-8 text-xs' : 'h-7 text-xs'}`} />
                                </div>
                                <div className="flex-1">
                                  <Label htmlFor={`fleet-gst-${vendor.vendor}`} className="text-xs">GST</Label>
                                  <Input id={`fleet-gst-${vendor.vendor}`} type="number" placeholder="0" value={fleetCosts[vendor.vendor]?.gst || ''} onChange={e => updateFleetCost(vendor.vendor, fleetCosts[vendor.vendor]?.cost || 0, parseFloat(e.target.value) || 0)} className={`${isMobile ? 'h-8 text-xs' : 'h-7 text-xs'}`} />
                                </div>
                              </div>
                              <div className="flex gap-2">
                                <Button variant="outline" size="sm" onClick={() => updateFleetCost(vendor.vendor, 0, 0)} className={`${isMobile ? 'h-8 text-xs flex-1' : 'h-6 text-xs flex-1'}`}>
                                  Set as Free
                                </Button>
                                <Button size="sm" onClick={() => saveFleetCost(vendor.vendor)} className={`${isMobile ? 'h-8 text-xs flex-1' : 'h-6 text-xs flex-1'}`}>
                                  Save
                                </Button>
                              </div>
                ...
                {/* Desktop Actions */}
                {!isMobile && !isReadOnly && <div className="space-y-2 pt-4 border-t">

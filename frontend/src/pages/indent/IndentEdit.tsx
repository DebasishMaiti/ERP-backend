...
export default function BoQEdit() {
  ...
  // Find the BoQ to edit
  const boqToEdit = mockData.boqs.find(boq => boq.id === id);
  ...
  // Form state - Initialize with existing BoQ data
  const [boqData, setBoqData] = useState({
    title: boqToEdit?.title || "",
    projectId: boqToEdit?.project || "",
    location: (boqToEdit as any)?.location || "",
    neededBy: (boqToEdit as any)?.neededBy || "",
    requester: boqToEdit?.createdBy || currentUser.name,
    notes: (boqToEdit as any)?.notes || "",
    status: boqToEdit?.status || "Draft"
  });

  // Initialize line items from existing BoQ data
  ...
  // Load existing line items on component mount
  useEffect(() => {
    if (boqToEdit?.items) {
      const existingLineItems: BoQLineItem[] = boqToEdit.items.map((item: any, index: number) => ({
  ...
  // Check if BoQ exists and is editable
  if (!boqToEdit) {
    return (
      <div>
  ...
  if (boqToEdit.status !== "Draft") {
    return (
      <div>
        <div className="container mx-auto px-4 py-6">
          <Card>
            <CardContent className="text-center py-8">
              <p className="text-muted-foreground">Only draft BoQs can be edited. This BoQ has status: {boqToEdit.status}</p>
              <Button className="mt-4" onClick={() => navigate("/indent/list")}>
                Back to BoQ List
              </Button>
            </CardContent>
  ...
  // Check if current user can edit BoQs
  ...
  const updateBoqData = useCallback((field: string, value: string) => {
    setBoqData(prev => ({ ...prev, [field]: value }));
  ...
  const validateForm = () => {
    ...
    if (!boqData.projectId) errors.push("Project selection is required");
  ...
  const saveDraft = useCallback(() => {
    ...
    if (boqData.title.trim() === "" || !boqData.projectId) {
      toast({
        title: "Error",
  ...
  const sendToCompare = useCallback(() => {
    ...
    // In real app, this would update status and notify purchaser
    setHasUnsavedChanges(false);
    toast({
      title: "Success",
  ...
  }, [boqData, lineItems, toast, navigate]);

  const handleCancel = () => {
    if (hasUnsavedChanges) {
      // AlertDialog will handle the confirmation
      return;
    }
    navigate("/indent/list");
  };
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => updateBoqData("projectId", value)}
                >
                  <SelectTrigger>
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t">
              ...
              <Badge variant="outline">BoQ #: {boqToEdit.number}</Badge>
              <Badge variant="outline">Created By: {boqToEdit.createdBy}</Badge>
              <Badge variant="outline">Created On: {new Date(boqToEdit.createdOn).toLocaleDateString()}</Badge>
              {boqData.projectId && (
                <Badge variant="outline">
                  Project: {projects.find(p => p.id === boqData.projectId)?.name || "Unknown"}
                </Badge>
              )}
            </div>
          </CardContent>
